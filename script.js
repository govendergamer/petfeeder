...
function loginUser(email, password) {
    if (email && password && password.length >= 8) {
        ...
        document.getElementById('sidebar').style.display = 'flex';
        updateUserProfileUI();
        addActivity('User logged in', 'fa-sign-in-alt', '#2d7d46');
...
function registerUser(firstName, lastName, email, password) {
    if (firstName && lastName && email && password && password.length >= 8) {
        ...
        document.getElementById('sidebar').style.display = 'flex';
        updateUserProfileUI();
        addActivity('New user registered', 'fa-user-plus', '#2d7d46');
...
function forgotPassword(email) {
    if (email) {
        ...
        addActivity('Password reset requested', 'fa-key', '#f59e0b');
        addNotification('Reset link sent to your email', 'info');
        return true;
    }
    return false;
}
...
function logoutUser() {
    if (confirm('Are you sure you want to logout?')) {
        ...
        addActivity('User logged out', 'fa-sign-out-alt', '#c62828');
        showAuthForm('login');
    }
}
...
// -------------------- FEEDING FUNCTIONS --------------------
function feedPet(amount, mode) {
    if (state.foodLevel <= 0) {
        addNotification('Food container is empty!', 'danger');
        return false;
    }
    if (amount > state.foodLevel) {
        addNotification(`Only ${Math.round(state.foodLevel)}g available`, 'warning');
        return false;
    }
    state.foodLevel = clamp(state.foodLevel - amount, 0, 100);
    updateFoodUI();
    ...
    addActivity(`Dispensed ${amount}g for ${petName}`, 'fa-utensils', '#e65100');
    addNotification(`Dispensed ${amount}g for ${petName}`, 'success');
    saveState();
    renderHistoryPanel();
    return true;
}
...
function init() {
    ...
    if (dom.feedPageDecBtn) {
        dom.feedPageDecBtn.addEventListener('click', () => {
            state.manualAmount = clamp(state.manualAmount - 5, 5, 100);
            if (dom.feedPageAmount) dom.feedPageAmount.textContent = state.manualAmount;
            if (dom.manualAmountDisplay) dom.manualAmountDisplay.textContent = state.manualAmount;
        });
    }
    if (dom.feedPageIncBtn) {
        dom.feedPageIncBtn.addEventListener('click', () => {
            state.manualAmount = clamp(state.manualAmount + 5, 5, 100);
            if (dom.feedPageAmount) dom.feedPageAmount.textContent = state.manualAmount;
            if (dom.manualAmountDisplay) dom.manualAmountDisplay.textContent = state.manualAmount;
        });
    ...
    if (dom.quickRecogBtn) {
        dom.quickRecogBtn.addEventListener('click', () => navigateTo('pet-profile'));
    }
    if (dom.quickScheduleBtn) {
        dom.quickScheduleBtn.addEventListener('click', () => navigateTo('schedule'));
    }
    if (dom.modalClose) dom.modalClose.addEventListener('click', closeMealModal);
    if (dom.mealForm) {
        dom.mealForm.addEventListener('submit', function(e) {
            e.preventDefault();
            const time = dom.mealTime.value;
            const amount = parseInt(dom.mealAmount.value);
            const days = Array.from(dom.dayCbs).filter(cb => cb.checked).map(cb => parseInt(cb.value));
            ...
            if (editIdx !== '') {
                editMeal(parseInt(editIdx), time, amount, days);
            } else {
    ...
    if (dom.notifBell) {
        dom.notifBell.addEventListener('click', () => navigateTo('notifications'));
    }
    if (dom.addPetBtn) {
        dom.addPetBtn.addEventListener('click', () => {
            const type = prompt('Pet type (Dog/Cat/Other):', 'Dog');
            if (type) addNewPet(type.toLowerCase());
        });
    }
    if (dom.addCatBtn) dom.addCatBtn.addEventListener('click', () => addNewPet('cat'));
    if (dom.addDogBtn) dom.addDogBtn.addEventListener('click', () => addNewPet('dog'));
    if (dom.gotoScheduleBtn) {
        dom.gotoScheduleBtn.addEventListener('click', () => navigateTo('schedule'));
    }
    if (dom.resetDemoBtn) {
    ...
    if (dom.autoDetectToggle) dom.autoDetectToggle.addEventListener('click', toggleAutoDetect);
    
    // Support buttons
    if (dom.supportEmailBtn) dom.supportEmailBtn.addEventListener('click', supportEmail);
    if (dom.supportCallBtn) dom.supportCallBtn.addEventListener('click', supportCall);
    if (dom.supportChatBtn) dom.supportChatBtn.addEventListener('click', supportChat);
    ...
    console.log('🐾 Smart Pet Feeder initialized');
    console.log(`📊 ${state.feedingHistory.length} feeding records loaded`);
    console.log(`🔔 ${state.notifications.length} notifications loaded`);
